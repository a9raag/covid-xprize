...
class XPrizePredictor(object):
    ...
    def __init__(self, path_to_model_weights, data_url):
        ...
        self.df = self.df[self.df.Date >= "2020-04-1"]
    ...
    def predict(self,
        ...
        # npis_df = npis_df[npis_df.Date >= "2020-03-01"]

        for column in NPI_COLUMNS:
            npis_df.update(npis_df.groupby("GeoID")[column].ffill().fillna(0))
            npis_df[column] = (npis_df[column] - npis_df[column].mean()) / npis_df[column].std(ddof=0)
        # TODO: try different intital conditions
    ...
    def _prepare_dataframe(self, data_url: str) -> pd.DataFrame:
        ...
        # Z normalisation on npi columns
        for column in NPI_COLUMNS:
...
if __name__ == '__main__':
    ...
    print("Saving model")
    # model_name = march_norm_NUM_TRIALS_LSTM_SIZE_LOOKBACK_DAYS_TOP_COUNTRIES_Dropout Rate
    model.save_weights("models/init_march_norm_lstm_5_32_21_30_d20.h5")
